...
let rec infer inf expr =
  begin match expr with
    ...
    | Expr.Abs(para_ident,body_expr) ->
      ...
      let body_type = infer inf body_expr in
      para_type @-> body_type
    | Expr.App(fun_expr,arg_expr) ->
      let fun_type = infer inf fun_expr in
      let arg_type = infer inf arg_expr in
    ...
    | Expr.LetFun(ident,fun_expr,body_expr) ->
      ...
      let fun_type = infer inf_fun fun_expr in begin
